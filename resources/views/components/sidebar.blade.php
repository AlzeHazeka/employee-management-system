<div x-data="{ open: true }" class="fixed h-full bg-blue-600 transition-all duration-300" :class="open ? 'w-40' : 'w-14'">
    <!-- Tombol Toggle -->
    ...
    <a class="mt-2 flex h-12 w-full items-center px-4" href="{{ route('dashboard') }}">
      ...
      <span x-show="open" class="ml-2 text-sm font-bold text-white">Employee Management</span>
    </a>
      ...
      <div class="mt-3 flex w-full flex-col items-center border-t border-gray-300">
        ...
        @if(auth()->user()->role == 'Admin')
            <div x-data="{ open: false }">
                <!-- Tombol Dropdown -->
                <button @click="open = !open" class="mt-2 flex h-12 w-full items-center rounded px-3 hover:bg-gray-300">
                    <svg class="h-6 w-6 stroke-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="white">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M5.121 17.804A4 4 0 0112 20a4 4 0 016.879-2.196M12 14a4 4 0 110-8 4 4 0 010 8z"/>
                    </svg>
                    <span class="ml-2 text-sm font-medium text-white">Rekap Presensi</span>
                    <svg class="ml-auto h-4 w-4 transition-transform transform"
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                    ...
                    <a href="{{ route('admin.presensi.by-date') }}" @click.stop class="block px-3 py-2 text-sm text-white hover:bg-gray-400 rounded">
                        Berdasarkan Tanggal
                    </a>
                    <a href="{{ route('admin.presensi.by-user') }}" @click.stop class="block px-3 py-2 text-sm text-white hover:bg-gray-400 rounded">
                        Berdasarkan Karyawan
                    </a>
                    <a href="{{ route('admin.presensi.rekap.presensi') }}" class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">
                        Rekap Presensi
                    </a>
                    <a href="{{ route('admin.presensi.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">
                        Presensi Manual
                    </a>
                </div>
            </div>
